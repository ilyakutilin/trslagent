...
- Proxy settings live in `[proxy]` (`PROXY__*`); the proxy `password` is env-only — never in TOML.
...
- **Proxy behavior**: Configurable via the `[proxy]` settings section (`PROXY__*` env vars): the `enabled` master switch (default `true`), an explicit `protocol`/`host`/`port`/`username` (+ `password` — env/.env only, never TOML), or fallback to the standard proxy env vars (`ALL_PROXY`/`HTTPS_PROXY`/`HTTP_PROXY`, upper- and lower-case). Resolution happens in `src/http_client.py:resolve_proxy_kwargs` (pure function; raises `ProxyConfigError` when the proxy is enabled but no explicit config and no env vars are found). `create_client()` accepts `proxy_settings: ProxySettings | None` (None = legacy `trust_env=True` behavior). An explicit `protocol` forces `trust_env=False` so env vars never interfere; a disabled proxy forces direct connections. SOCKS protocols require the `socksio` package (`httpx[socks]`); without it, `httpx.AsyncClient.__init__` raises `ImportError` before any request is sent. When the env-var fallback is used, the resolved variable names and values are logged (passwords embedded in proxy URLs are masked via `_mask_proxy_url`). `ProxyConfigError` is handled at the CLI boundary (`src/cli.py:_run_or_exit`): it is logged and the process exits with code 1 — never a raw traceback. In email mode the same error surfaces as an error reply.
- **Proxy threading**: `cfg.proxy` flows explicitly into all httpx clients: `main()` → `create_client`/`LLM`; `serve()` creates the shared email server client with `cfg.proxy`; email_processor owned clients accept `proxy_settings`.
...
- **Config**: `[geoblock]` section with one key `countries: list[str]` (default `[]` = disabled); env var `GEOBLOCK__COUNTRIES` (JSON list, e.g. `["RU","BY"]`). Values are ISO 3166 alpha2 (`"US"`), alpha3 (`"USA"`), numeric (`"840"`) codes or full English names (must match the `iso3166` package's names, e.g. `"Russian Federation"`), case-insensitive; validated at Settings construction by `parse_geoblock_countries` in `src/config.py` (pydantic `field_validator` on `GeoblockSettings.countries`, `iso3166` package) and normalized to canonical uppercase alpha2 codes (deduplicated, order preserved). Unknown values → `ValidationError` → the `ValidationError` handlers in `src/cli.py` log `Invalid configuration: ...` and exit 1 (both one-shot runs and `serve-emails` startup).
- **Module**: `src/geoblock.py` — `verify_ip_not_geoblocked(blocked_countries, *, proxy_settings, client, timeout)` queries the checkers in `CHECKERS` concurrently (`ip-api.com` `http://ip-api.com/json/` (HTTP-only on the free tier), `ipinfo.io`, `api.country.is`), `CHECKER_TIMEOUT = 5.0` per request, `MIN_CLEAN_CONFIRMATIONS = 2`; raises `GeoblockError` (subclass of `RuntimeError`).
- **Fail-closed decision matrix**: any checker reporting a blocked country → block; otherwise proceed only if ≥2 of 3 checkers confirm a non-blocked country (one checker may fail/time out); <2 clean confirmations → block ("unable to verify"). The owned client is created via `create_client(proxy_settings=...)` and closed in a `finally` even when the check raises.
- **Where it runs**: in `src/main.py:main()` once per request cycle — once per CLI launch (translation or review) and once per processed inbound email (the email flow calls `main()` per email) — before any LLM call. Skipped when `cfg.geoblock.countries` is empty or `print_prompt_only` is set; `--match-glossary`/match-glossary mode never goes through `main()`. **Email merge**: in `src/email_processor.py:build_settings_from_email`, the server's `geoblock.countries` are merged (deduplicated, order preserved) with any `[geoblock]` section from an attached `config.toml`, so an attached config can only extend the server policy, never remove it.
- **Proxy threading**: geo requests go through the same proxy as the LLM (`verify_ip_not_geoblocked(..., proxy_settings=cfg.proxy)`), so the **proxied IP** is what gets verified; direct connections when the proxy is disabled.
- **Error surfaces**: `GeoblockError` is handled at the CLI boundary (`src/cli.py:_run_or_exit` — logs `Geoblock check failed: ...`, exit code 1) and in `src/email_server.py:_process_inbound` (logs and sends a threaded error reply with the block reason).
- **Testing**: `GEOBLOCK__COUNTRIES` is cleared by the autouse `_no_dotenv` fixture in `tests/conftest.py`; geoblock tests live in `tests/test_geoblock.py`, plus coverage in `test_config.py`, `test_cli.py`, `test_main.py`, `test_email_server.py`, `test_email_processor.py`.
...
- **Proxy env vars are cleared for tests**: the autouse `_clear_proxy_env` fixture in `tests/conftest.py` deletes `ALL_PROXY`/`HTTPS_PROXY`/`HTTP_PROXY` (upper- and lower-case) for every test, so developer shells with proxies configured don't break httpx-based tests. Because the default `ProxySettings` is `enabled=True` with no `protocol`, tests that reach the real `create_client` with real `cfg.proxy` must set `proxy_settings=ProxySettings(enabled=False)` (or equivalent) or set proxy env vars explicitly — see the `_no_proxy` fixture in `tests/test_main.py`. Proxy-specific behavior tests live in `tests/test_http_client.py::TestProxyResolution`. The fixture exists for test isolation — do not write proxy-behavior tests relying on it; set up proxy env vars explicitly per-test if needed.
- **`.env` is disabled for tests**: the autouse `_no_dotenv` fixture in `tests/conftest.py` patches `Settings.model_config` with `env_file=None` and clears the `COST__*` env vars. The developer's `.env` (e.g. `COST__GENERATION_INFO_URL`) can therefore NEVER inject real network behavior (cost fetches, proxies, keys) into the suite — before this guard existed, tests in `test_main.py` made real HTTP requests to OpenRouter with real retry backoff, taking ~3.5 minutes. Tests that need env-file behavior override `Settings.model_config` explicitly (see `tests/test_email_settings.py`). Do not rely on `.env` values in tests; construct settings explicitly or set env vars per-test.
- **Cost resolution in pipeline tests**: `resolve_and_log_cost` is imported into `src/pipeline/translation.py` and `src/pipeline/review.py` at module load, so `mocker.patch("src.llm.resolve_and_log_cost")` does NOT intercept pipeline calls — patch the pipeline module attributes instead. With the `_no_dotenv` guard, the real function early-returns anyway (no `generation_info_url`).
