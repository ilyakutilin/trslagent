...
class TestBuildAutomaton:
    def test_terms_inserted_can_iterate_matches(self, empty_matcher):
        automaton = empty_matcher.build_automaton(["flow meter", "pressure valve"])

        matches = list(automaton.iter("the flow meter is broken"))
        found = [m[1] for m in matches]
        assert "flow meter" in found

    def test_no_matches_in_text(self, empty_matcher):
        automaton = empty_matcher.build_automaton(["flow meter"])
...
class TestFindGlossaryEntriesInText:
    def test_no_matches(self, lemmatizer, empty_matcher):
        automaton = empty_matcher.build_automaton(["flow meter"])

        result = empty_matcher.find_glossary_entries_in_text(
    ...
    def test_empty_text(self, lemmatizer, empty_matcher):
        automaton = empty_matcher.build_automaton(["flow meter"])
    ...
    def test_single_exact_match(self, lemmatizer, empty_matcher):
        automaton = empty_matcher.build_automaton(["flow meter"])
    ...
    def test_overlapping_matches_longest_wins(self, lemmatizer, empty_matcher):
        automaton = empty_matcher.build_automaton(["flow", "flow meter"])
    ...
    def test_multi_word_term(self, lemmatizer, empty_matcher):
        automaton = empty_matcher.build_automaton(["pressure relief valve"])
    ...
    def test_unsupported_language_returns_empty(self, empty_matcher):
        unsupported = FakeLemmatizer(supported=frozenset())
        automaton = empty_matcher.build_automaton(["flow meter"])
        ...
        result = empty_matcher.find_glossary_entries_in_text(
            ...
            automaton=automaton,
            lang=Lang("fr"),
            lemmatizer=unsupported,
        )
        assert result == []

    def test_token_boundary_subword_not_matched(self, lemmatizer, empty_matcher):
        automaton = empty_matcher.build_automaton(["flow"])
    ...
    def test_token_boundary_exact_token_matched(self, lemmatizer, empty_matcher):
        automaton = empty_matcher.build_automaton(["flow"])
...
class TestMatch:
    ...
    def test_empty_text(self, lemmatizer):
        ...
        result = matcher.match("", Lang("en"), lemmatizer=lemmatizer)
        assert result == []
    ...
    def test_multi_entry_same_lemma_deduplicated(self, lemmatizer):
        e1 = _make_entry(1, "flow")
    ...
    def test_with_default_lemmatizer(self, mocker, lemmatizer):
        mock_lemma_cls = mocker.patch(
            "src.glossary.matcher.Lemmatizer", return_value=lemmatizer
        )
        e1 = _make_entry(1, "flow meter")
        matcher = TermMatcher([e1])
